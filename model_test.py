...
def model_check(data_dict, csvfile):
    ...
    if(algo_choice==1):
        return classification(prepped_df,X_train, X_test, y_train, y_test, epsil_val, datanorm_val)
    elif(algo_choice==2):
        return regression(prepped_df,X_train, X_test, y_train, y_test, epsil_val)
    elif(algo_choice==3):
        return clustering(prepped_df,X_train, X_test, y_train, y_test,cluster_no)
